...
#   condicion numF - 1 --> $t6
#   
#   datos[f * numC + indCol] --> $t7
#   datos[(f + 1) * numC + indCol] --> $t8
#
#   double delta --> $f4
#   double elem1 --> $f6
#   double elem2 --> $f8
#   condicion elem2 + delta --> $f10
#   guardamos 1.25 --> $f16
###############################################################################
reparte_columna:
...
#   double delta = 0.25;
  ...
  # Declaramos numF - 1 antes del bucle
  addi $t6, $t2, -1

#   for(int f = 0; f < (numF -1); f++) {
for_reparte_columna:
  bge $t5, $t6, for_reparte_columna_fin
...
#     delta += 1.25;
  ...
  addi $t5, $t5, 1
  b for_reparte_columna

#   }
for_reparte_columna_fin:
...
#   datos[0 * numC + c] --> $t1
#   cargamoss -3.5 --> $f4 
#   valor de totIntercambio en double --> $f6
###############################################################################
organiza_columnas_cota:
  # PUSH -> $ra, $s0-$s6, $f20, $f22, $f24 --> 8 * 4 + 8 * 3 = 56
  addi $sp, $sp, -56
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
for_organiza_columnas_cota:
  bgt $s6, $s2, for_organiza_columnas_cota_fin
...
if_organiza_columnas_cota_cuerpo_fin:
...
###############################################################################
main:
  # PUSH -> $ra, $s0, $s1, $s2, $s3, $f20, $f22 --> 5 * 4 + 8 * 2 = 36
  # Reservamos 40 para que sea múltiplo de 8
  addi $sp, $sp, -40
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
#         if ((matT < 0) || (matT >= NUM_MATRICES)) {
if_caso_1:
  blt $t0, 0, if_caso_1_cuerpo
  bge $t0, NUM_MATRICES, if_caso_1_cuerpo
  b if_caso_1_cuerpo_fin
