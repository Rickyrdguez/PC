...
#   int numCambios --> $v0
#
# Función leaf --> NO modifica $ra --> NO llama a otras funciones 
#
# Tabla de parámetros:
#   double* v1 --> $a0 --> $t0
#   double* v2 --> $a1 --> $t1
...
#   double elem2 --> $f8
#
#   guardar el valor -1.5 --> $f10 
#   guardar valor 0.25 --> $f16
#
#   condicion numElem - 1 --> $t6
#   
#   posicion ind1 --> $t7
#   posicion ind2 --> $t8
#######################################################################
...
#   for(int ind1 = (numElem - 1); ind1 >= 0; ind1--) {
for_reparte_vectores:
  blt $t4, 0, for_reparte_vectores_fin
...
if_reparte_vectores:
  c.lt.d $f4, $f6
  bc1t if_reparte_vectores_cuerpo

  c.lt.d $f6, $f10
  bc1t if_reparte_vectores_cuerpo

  b else_reparte_vectores

if_reparte_vectores_cuerpo:
#       double elem2 = v2[ind2];
  mul $t8, $t5, tamD
  add $t8, $t8, $t1
  l.d $f8, 0($t8)
...
#       numCambios++;
  ...
  b else_reparte_vectores_fin

if_reparte_vectores_cuerpo_fin:
#     } else {
else_reparte_vectores:
...
#     }
else_reparte_vectores_fin:

  # Guardamos 0.25 en $f16
  li.d $f16, 0.25
...
#     cota += 0.25;
  ...
  addi $t4, $t4, -1
  b for_reparte_vectores

#   }
for_reparte_vectores_fin:
...
#   double cota --> $f12
# Parámetros de salida:
#   double acumulado  * totIntercambios --> $v0
#
# SÍ llama a otras funciones --> SÍ modifica $ra --> SÍ usa pila
#
# Tabla de paráemtros:
#   strcutMat* mat --> $a0 
#   int inicial --> $a1 --> $s0
...
#
#   datos[inicial * numC + 0]--> $t3
#   datos[f * numC + 0] --> $t4
#   cargar datos[f * numC + 0] en double --> $f4
#   totIntercambios en double --> $f6
########################################################################
organiza_filas:
  # PUSH -> $ra, $s0-$s5, $f20, $f22 -> 8 * 4 + 8 * 2 = 48
  addi $sp, $sp, -48
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
#   double* f1 = &(datos[inicial * numC + 0]);
  ...
  #  Inicializamos f = inicial + 1 antes del bucle
  addi $s5, $s0, 1

#   for(int f = (inicial+1); f <= final; f++) {
for_organiza_filas:
  bgt $s5, $s1, for_organiza_filas_fin
...
#     totIntercambios += numc;
  ...
  addi $s5, $s5, 1
  b for_organiza_filas

#   }
for_organiza_filas_fin:

  # Pasamos totIntercambios a double
  mtc1 $s2, $f6
  cvt.d.w $f6, $f6
...
#
#   int matT --> $t0
#   para matrices[elegida] --> $t1
########################################################################
main:
  # PUSH -> $ra, $s0-$s4, $f20, $f22 -> 6 * 4 + 8 * 2 = 40
  addi $sp, $sp, -40
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
