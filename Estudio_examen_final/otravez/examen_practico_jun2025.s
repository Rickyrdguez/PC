...
#   double delta = 0.25;
  ...
  # Inicializamos f = 0 antes del bucle
  li $t5, 0

  # Usamos $t8 para numF -1
  addi $t8, $t2, -1

#   for(int f = 0; f < (numF -1); f++) {
for_f_reparte:
  bge $t5, $t8, for_f_reparte_fin
...
#   int inicial --> $a1 --> $s1
#   int final --> $a2 --> $s2
#   int totIntercambios --> $s3 --> double -> $f6
#   int numC --> $s4
#   double* datos --> $s5
#   int c --> $s6
#   
...
#
#   calculos momentaneos --> $t0
#   -3.5 --> $f4
###############################################################################
organiza_columnas_cota:
  # PUSH --> $ra, $s0-$s6, $f20,22,23 = 8 * 4 + 8 * 3 = 56 multiplo de 8
  addi $sp, $sp, -56
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
#   double* datos = mat->elementos;
  ...
  # Declaramos c = inicial antes del bucle
  move $s6, $s1
...
###############################################################################
main:
  # PUSH --> $ra, $s0-$s5, $f20, $f22 = 7 * 4 + 8 * 2 = 48 --> múltiplo de 8 
  addi $sp, $sp, -48
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
#         if ((matT < 0) || (matT >= NUM_MATRICES)) {
if_caso1:
  blt $s2, 0, if_caso1_cuerpo
  bge $s2, NUM_MATRICES, if_caso1_cuerpo
  b if_caso1_fin
