...
#   guardamos -1.5 --> $f10
#   cargar 0.25 --> $f16
#   Cálculo de posiciones para v1 --> $t6 
#   Cálculo de posiciones para v2 --> $t7
#   vi[ind2] --> $t8
#######################################################################
reparte_vectores:
  move $t0, $a0
...
#   int f2 --> $t4
#
#   datos[f * numC + 0] --> $f4 haciendo uso tmb de $t1
#   totIntercambios en double --> $f6
#   Cálculos momentaneos --> $t2
#   condicion inicial+1 --> $t3
#######################################################################
organiza_filas:

  # PUSH -> $ra, $s0-$s6, $f20, $f22 -> 8 * 4 + 8 * 2 = 48 
  addi $sp, $sp, -48
  sw $ra, 0($sp)
  sw $s0, 4($sp)
  sw $s1, 8($sp)
...
#   double* f1 = &(datos[inicial * numC + 0]);
  ...
  # Inicializamos f = inicial+1
  addi $t3, $s0, 1
  move $s6, $t3

#   for(int f = (inicial+1); f <= final; f++) {
for_organiza_filas:
  bgt $s6, $s1, for_organiza_filas_fin
